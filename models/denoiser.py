...
class MDM_FastSpeech(nn.Module):
    def __init__(self, args):
        ...
        self.embed_text = nn.Linear(768*4, self.latent_dim)
    ...
    def forward(self, x, timesteps, fastspeech, fastspeech_batch, y=None,uncond_info=False):
        """
        ...
        # inject fastspeech2 
        fastspeech_output = fastspeech(*(fastspeech_batch[2:]))
        audio_feat = fastspeech_output[0] # sythesized text-driven audio feature that incorporates pitch, duration, energy, etc
        audio_feat = self.WavEncoder(audio_feat).permute(1, 0, 2) # (64, 40, 256)
        # audio_feat = self.WavEncoder(y['audio']).permute(1, 0, 2) # (64, 40, 256)
        text_feat = self.text_pre_encoder_body(y['word'])
        text_feat = self.text_encoder_body(text_feat).permute(1, 0, 2) # (64, 40, 256)

        at_feat = torch.cat([audio_feat,text_feat],dim=2)
        at_feat = self.mix_audio_text(at_feat) # (64, 40, 256)

        # This part is test for timm transformer blocks
        x = x.reshape(bs, njoints * nfeats, 1, nframes) 
        # self-attention
        x_ = self.input_process(x)  # [40, 768, 1, 64] -> [64, 40, 512]
        ...
        xseq = self.input_process2(xseq)    # (64, 40, 1280) -> (64, 40, 512)
        ...
        return output[...,:noise_length], fastspeech_output
