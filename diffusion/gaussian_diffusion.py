...
class GaussianDiffusion:
    ...
    def training_losses_fastspeech(self, model, fastspeech, fastspeech_batch, x_start, t, model_kwargs=None, noise=None, dataset=None):
        """
        Compute training losses for a single timestep, include fastspeech as audio feature embedding

        :param model: the model to evaluate loss on.
        :param fastspeech: the fastspeech model to evaluate loss on.
        :param x_start: the [N x C x ...] tensor of inputs.
        :param t: a batch of timestep indices.
        :param model_kwargs: if not None, a dict of extra keyword arguments to
            pass to the model. This can be used for conditioning.
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:     # LossType.MSE
            ...
            # model_output = model(x_t, self._scale_timesteps(t), **model_kwargs)
            model_output, fastspeech_output = model(x_t, self._scale_timesteps(t), fastspeech, fastspeech_batch, **model_kwargs)

            # add fastspeech output to terms
            terms['fastspeech_out'] = fastspeech_output
            if self.model_var_type in [     # ModelVarType.FIXED_SMALL: 2
                ModelVarType.LEARNED,
                ModelVarType.LEARNED_RANGE,
            ]:
